...
internal static class Twain
{
    ...
    public static Dictionary<string, object> SelfTest()
    {
        ...
        // Служба Windows. Спрашиваем её и отсюда, и из главной программы,
        // а рядом кладём разбор по шагам и проверку по реестру: пустой
        // список должен объяснять сам себя, а не оставлять место догадкам
        try
        {
            report["wiaReady"] = Wia.Ready();
            report["wiaService"] = Wia.ServiceState();
            report["wiaScanners"] = Wia.List().Select(d => d.Name).ToList();

            // Пошаговый рассказ: сколько устройств насчитала служба,
            // какие пропущены и почему. Без него пустой список
            // приходилось объяснять догадками
            report["wiaWalk"] = Wia.Log;
