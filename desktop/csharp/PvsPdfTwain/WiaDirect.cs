...
internal static class WiaDirect
{
    ...
    public static string? ScanToFile(string path, string deviceId = "")
    {
        ...
        // ВАЖНО про порядок. По новым правилам службы нужная команда
        // стоит в таблице на другом месте, чем по старым. Раньше мы
        // звали её по одному и тому же номеру у обоих — и по новым
        // правилам попадали мимо, в чужую функцию. Это рушило весь
        // помощник целиком (AccessViolation), из-за чего остальные
        // способы съёмки даже не пробовались.
        //
        // Обычный catch тут не спасает: порча памяти валит процесс
        // раньше, чем мы успеем её перехватить. Поэтому идём только
        // старыми правилами, где расположение команды известно точно
