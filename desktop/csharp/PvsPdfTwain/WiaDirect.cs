    ...
    public static string? ScanToFile(string path, string deviceId = "")
    {
        ...
        foreach (var pair in new[] { (CLSID_NEW, IID_DEVMGR2), (CLSID_OLD, IID_DEVMGR) })
        {
            try
            {
                ...
                // Интерфейс просим СТАРЫЙ у обеих служб.
                //
                // Это важно для безопасности: расположение нужной
                // команды в таблице мы знаем точно только для старого
                // интерфейса. Попытка звать её по тому же номеру
                // у нового интерфейса однажды уже уронила помощник.
                // Новая служба старый интерфейс обычно отдаёт — тогда
                // мы получаем и живую службу, и знакомую таблицу
                var iid = IID_DEVMGR;
