...
internal static class WiaDirect
{
    ...
    static string Poko(Guid id) => id == CLSID_NEW ? "правила новые" : "правила старые";
    ...
    public static string? ScanToFile(string path, string deviceId = "")
    {
        ...
        // Сначала новое поколение службы, затем старое.
        //
        // Раньше здесь было только старое — и оно на этом компьютере
        // отказывает с кодом 80004002, ровно как при поиске. Поиск это
        // давно учёл и работает по новым правилам, а съёмка упрямо
        // просила старые. Набор полей у обоих поколений одинаков,
        // поэтому просто пробуем оба
        // Порядок и способ — те же, что в поиске, который на этом
        // компьютере РАБОТАЕТ.
        //
        // Отчёт показал главное: старые правила тут мертвы («служба
        // не создана, код 80004002»), а новые прекрасно находят
        // аппарат («устройств 1»). Съёмка же упрямо просила только
        // старые — и потому не начиналась вовсе.
        //
        // Берём приём поиска целиком: сначала службу «в общем виде»,
        // затем отдельным шагом уточняем интерфейс. Так удаётся
        // договориться со службой, которая капризничает при прямом
        // запросе. Новые правила идут первыми — они здесь живые
        // Только новое поколение: набор полей команды, которым мы
        // снимаем, есть лишь у него. У старой службы та же команда
        // принимает СОВСЕМ другие поля, и зов по её таблице означает
        // порчу памяти, а не отказ
        foreach (var pair in new[] { (CLSID_NEW, IID_DEVMGR2) })
        {
            try
            {
                var id = pair.Item1;
                ...
                // У каждой службы просим ЕЁ СОБСТВЕННЫЙ интерфейс.
                //
                // Прошлый раз я просил старый интерфейс у обеих — и новая
                // служба отказала («интерфейс не отдан»), хотя именно она
                // на этом компьютере живая и видит аппарат. Нельзя просить
                // у нового поколения удостоверение старого образца.
                //
                // Опасений за таблицу команд нет: у обоих поколений
                // команда съёмки стоит на одном и том же, седьмом месте —
                // порядок команд в начале таблицы у них совпадает
                var iid = pair.Item2;
                ...
                if (hr == 0 && raw != IntPtr.Zero)
                {
                    ...
                    if (ask == 0 && typed != IntPtr.Zero)
                    {
                        Marshal.Release(raw);
                        raw = typed;
                        Log.Add($"прямая съёмка ({Poko(pair.Item1)}): интерфейс получен");
                    }
                    else
                    {
                        Log.Add($"прямая съёмка ({Poko(pair.Item1)}): интерфейс не отдан (код {ask:X8})");
                        Marshal.Release(raw);
                        raw = IntPtr.Zero;
                    }
                }

                // Не вышло «в общем виде» — пробуем сразу с интерфейсом
                if (raw == IntPtr.Zero)
                    hr = CoCreateInstance(ref id, IntPtr.Zero, INPROC_SERVER | LOCAL_SERVER, ref iid, out raw);

                if (hr != 0 || raw == IntPtr.Zero)
                {
                    Log.Add($"прямая съёмка ({Poko(pair.Item1)}): служба не создана (код {hr:X8})");
                    last = hr;
                    continue;
                }

                Log.Add($"прямая съёмка ({Poko(pair.Item1)}): аппарат «{deviceId}»");

                // Зовём напрямую: посредник .NET на описании службы
                // спотыкается — та же причина, что и в поиске
                // Место команды в таблице зависит от поколения службы.
                //
                // Я ошибся, поручившись, что оно одинаково. Сверил по
                // описаниям выше: у СТАРОЙ службы съёмка идёт пятой
                // (место 7), у НОВОЙ — восьмой (место 10). Мы же звали
                // место 7 у обеих: у новой там лежит совсем другая
                // команда — отсюда и порча памяти.
                //
                // Набор полей, который мы передаём, принадлежит НОВОЙ
                // службе: у старой он совершенно другой. Поэтому этим
                // путём снимаем только новой — старую сюда не пускаем
                var table = Marshal.ReadIntPtr(raw);
                var getImageDlg = Marshal.GetDelegateForFunctionPointer<GetImageDlgCall>(
                    Marshal.ReadIntPtr(table, 10 * IntPtr.Size));
                ...
                // Windows вместо окна производителя.
                //
                // Общее окно Windows я убрал с первого места. На МФУ
                // с памятью (Kyocera и подобные) оно само подставляет
                // подачу «Хранилище сканера» — то есть предлагает взять
                // готовый файл из памяти аппарата, а не снять лист.
                // Память пуста, «Сканировать» ничего не делает, и окно
                // просто стоит. Список подачи там куцый, и стекло
                // с автоподатчиком в нём порой не предложены вовсе.
                //
                // Окно самого драйвера Kyocera таких подмен не делает:
                // оно сразу предлагает стекло и автоподатчик. Его
                // и спрашиваем первым, общее оставляем про запас
