...
internal static class Scanner
{
    ...
    static string DiagnoseCore()
    {
        ...
        // Помощник есть, но только одной разрядности. Windows держит
        // отдельные списки 32- и 64-разрядных драйверов, и половина
        // аппаратов в таком случае остаётся невидимой
        if (twain && !(File.Exists(TwainBridge.ExePath()) && File.Exists(TwainBridge.ExePath64())))
            return "Установлена не полная версия помощника сканирования: часть сканеров (в том числе Kyocera и другие современные МФУ) остаётся невидимой. Переустановите программу свежим установщиком.";
