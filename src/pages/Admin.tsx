...
const statusOf = (l: License) => {
  if (l.status === 'blocked') return { label: 'Заблокирована', cls: 'bg-destructive text-destructive-foreground' };
  if (l.valid_until < today()) return { label: 'Истекла', cls: 'bg-foreground text-background' };
...
const Admin = () => {
  ...
  const [stats, setStats] = useState<Stats>({ total: 0, active: 0, expired: 0, blocked: 0 });
  const [search, setSearch] = useState('');
  const [loading, setLoading] = useState(false);
  const [form, setForm] = useState<{ open: boolean; item: License | null }>({ open: false, item: null });
      ...
      <main className="mx-auto max-w-[1280px] px-5 py-8">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3">
          <div className="flex flex-1 items-center border border-border bg-background">
            ...
            />
            <button className="px-3 py-3 text-primary hover:bg-card" onClick={() => load(search)}>
              <Icon name="Search" size={16} />
            </button>
          </div>
          <button className="btn-block" onClick={() => setForm({ open: true, item: null })}>
          ...
          <table className="w-full min-w-[820px] border-collapse text-left">
            <thead>
              <tr className="border-b border-border bg-card">
                {['Организация', 'Ключ активации', 'Действует до', 'Мест', 'Проверок', 'Статус', ''].map((h) => (
                  <th
                    key={h}
            ...
            <tbody>
              ...
              {!loading &&
                items.map((l) => {
                  const s = statusOf(l);
                  return (
                    <tr key={l.id} className="border-b border-border transition-colors last:border-0 hover:bg-card">
                      <td className="px-4 py-3">
                        <div className="font-head text-[0.9rem] font-bold">{l.org_name}</div>
                        {l.contact && (
                          <div className="text-[0.78rem] text-muted-foreground">{l.contact}</div>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <button
                        ...
                        >
                          {l.license_key}
                          <Icon name="Copy" size={13} className="opacity-0 group-hover:opacity-60" />
                        </button>
                      </td>
                      <td className="px-4 py-3 text-[0.86rem]">{l.valid_until}</td>
                      <td className="px-4 py-3 text-[0.86rem]">{l.seats}</td>
                      <td className="px-4 py-3 text-[0.86rem] text-muted-foreground">{l.activations}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-block px-2 py-1 font-head text-[0.68rem] font-bold uppercase tracking-[0.08em] ${s.cls}`}>
                          {s.label}
                        </span>
                      </td>
                      <td className="px-4 py-3">
